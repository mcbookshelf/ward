...
public final class DataCoverage {
	...
	public static MapCodec<? extends LootItemCondition> wrapCondition(MapCodec<? extends LootItemCondition> codec) {
		return new RecordingCodec<LootItemCondition>(codec, "condition", conditions, RecordingCondition::new);
	}
	...
	public static MapCodec<? extends LootPoolEntryContainer> wrapEntry(MapCodec<? extends LootPoolEntryContainer> codec) {
		return new RecordingCodec<LootPoolEntryContainer>(codec, "type", runs, (entry, wrapper, counts) -> {
	...
	public static MapCodec<? extends LootItemFunction> wrapFunction(MapCodec<? extends LootItemFunction> codec) {
		return new RecordingCodec<LootItemFunction>(codec, "function", runs, RecordingFunction::new);
	}

	public static MapCodec<? extends NumberProvider> wrapNumberProvider(MapCodec<? extends NumberProvider> codec) {
		return new RecordingCodec<NumberProvider>(codec, "type", runs, RecordingNumberProvider::new);
	}

	public static MapCodec<? extends SlotSource> wrapSlotSource(MapCodec<? extends SlotSource> codec) {
		return new RecordingCodec<SlotSource>(codec, "type", runs, RecordingSlotSource::new);
	}
	...
	public static void beginElement(ResourceKey<?> key, Resource resource, Object json) {
		beginElement(key.registry().toString(), key.identifier().toString(), resource, json);
	}

	public static void beginElement(String registry, String element, Resource resource, Object json) {
		if (CoverageRecorder.isEnabled()
				&& resource.sourcePackId().startsWith(DEPLOYED_PACK_PREFIX)
				&& json instanceof JsonElement root) {
			CURRENT.set(new Provenance(registry, element, root));
		}
	}
	 ...
	 * Decoding resolves the node through the dispatch element ({@code typeKey}) and hands the value to {@code wrap}; encoding unwraps delegates back to the original.
	 */
	private static final class RecordingCodec<A> extends MapCodec<A> {
		...
		RecordingCodec(MapCodec<? extends A> inner, String typeKey, NodeTable nodes, Wrap<A> wrap) {
		...
		public <T> DataResult<A> decode(DynamicOps<T> ops, MapLike<T> input) {
			return this.inner.decode(ops, input).map(value -> {
				int[] counts = counters(input.get(this.typeKey));
	...
	private static final class RecordingNumberProvider extends Recording<NumberProvider> implements NumberProvider {
		RecordingNumberProvider(NumberProvider inner, MapCodec<NumberProvider> codec, int[] counts) {
			super(inner, codec, counts);
		}
		...
		public float getFloat(LootContext context) {
			hit();
			return this.inner.getFloat(context);
		}

		/**
		 * Delegated rather than left to the default, which would round the float: providers like uniform pick integers differently.
		 */
		@Override
		public int getInt(LootContext context) {
			hit();
			return this.inner.getInt(context);
		}
