		...
		root.then(Commands.literal("blocks").then(Commands.argument("start", BlockPosArgument.blockPos())
				.then(Commands.argument("end", BlockPosArgument.blockPos())
						.then(Commands.argument("destination", BlockPosArgument.blockPos())
								.then(Commands.literal("all").executes(ctx -> run(ctx, assertion, false)))
								.then(Commands.literal("masked").executes(ctx -> run(ctx, assertion, true)))))));
	}

	private static int run(CommandContext<CommandSourceStack> context, Context assertion, boolean skipAir) throws CommandSyntaxException {
